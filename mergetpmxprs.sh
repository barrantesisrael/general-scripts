#!/bin/sh
# mergetpmxprs.sh -- merges TPM values from all XPRS files inside the current directory
# usage: mergetpmxprs.sh
# 2015-05-26 isradelacon@gmail.com

# TOTAL counts from XPRS files
for file in *.xprs; do cat $file | cut -f2,15 > $file.short; done
...
perl -e 'print "write.table (alldata, file=\"alltpmxprs.tsv\", row.names=FALSE, col.names=TRUE, quote=FALSE, sep=\"\\t\")\n";' >> mergescript.R
